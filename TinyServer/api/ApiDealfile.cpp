...
int handleShareFile(string &user, string &md5, string &filename)
{
    ...
    */
    ...
    cout << "fileid: " << fileid << ", ZsetExit: " << ret2<<endl;
    ...
    */
    ...
    {
        //插入记录
        sprintf(sql_cmd, "insert into user_file_count (user, count) values('%s', %d)", "share_file_count", 1);
        if (!pDBConn->ExecuteCreate(sql_cmd))
        {
            LOG_ERROR (sql_cmd , " 操作失败");
            ret = -1;
...
int removeFileFromFastDfs(const char *fileid)
{
    ...
    ret = system(cmd);
    cout << "removeFileFromFastDfs ret = " << ret<<endl;
...
int handleDeleteFile(string &user, string &md5, string &filename)
{
    ...
    */
    ...
    {                   //===2、如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
        ...
        if (ret2 == 0)
        {
            cout << "GetResultOneCount share  = " << shared_status<<endl;
            if(shared_status)
            {
                is_shared = 1;
            }
    ...
    count = 0;
    ret2 = GetResultOneCount(pDBConn, sql_cmd, count); //执行sql语句
    cout << "ret2: " << ret2 << ", count:" << count<<endl;
    if (ret2 != 0)
    {  
        LOG_ERROR ( sql_cmd , "操作失败");
        ret = -1;
    ...
    if (count == 0) //说明没有用户引用此文件，需要在storage删除此文件
    {
        ...
        ApiDealfileInit("/etc/fdfs/client.conf");
        ret2 = removeFileFromFastDfs(fileid.c_str());
        if (ret2 != 0)
        {
            cout << "removeFileFromFastDfs err: " << ret2<<endl;
            ret = -1;
            goto END;
        }
    }
...
int ApiDealfile(string &url, string &post_data, string &str_json)
{
    ...
    cout << "user_name:" << user_name << ", token:" << token << ", md5:" << md5 << ", filename:" << filename<<endl;
