...
int CDBPool::Init(const char* pool_name, const char* db_server_ip, uint16_t db_server_port,
	...
	if (m_free_list.empty())		// 当没有连接可以用时
	{
		...
		if (m_db_cur_conn_cnt >= m_db_max_conn_cnt)
		{
			...
			if(timeout_ms <= 0)		// 死等，直到有连接可以用 或者 连接池要退出
			{
				...
				{					
					// 
					//当前连接数量小于最大连接数量 或者请求释放连接池时解除 条件变量的阻塞
...
int CDBManager::Init()
{
	...
	if (!db_instances)
	{
		cout << "not configure DBInstances";
		return 1;
	}
	...
	for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++)
	{
		...
		cout << "db_host:" << db_host << ", db_port:" << str_db_port << ", db_dbname:" << db_dbname
				 << ", db_username:" << db_username << ", db_password:" << db_password;

		if (!db_host || !str_db_port || !db_dbname || !db_username || !db_password || !str_maxconncnt)
		{
			cout << "not configure db instance: " << pool_name;
			return 2;
		}
		...
		if (pDBPool->Init())
		{
			cout << "init db instance failed: " << pool_name;
			return 3;
		}
...
int CDBManager::addDBCPool(CDBPool* pDBPool)
{
	m_dbpool_map.insert(make_pair(pDBPool->GetPoolName(), pDBPool));
}
